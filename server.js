...
async function getSession(id) {
  ...
  const questions = (await db(
    "SELECT id, position, text, type, options_json AS options FROM questions WHERE session_id=$1 ORDER BY position",
    [id]
  )).rows;
  return { ...s, questions };
}
...
function publicQuestion(q) {
  return q ? { id:q.id, position:q.position, text:q.text, type:q.type, options:q.options } : null;
}
...
async function resultsFor(sessionId) {
  ...
  for (const q of s.questions) {
    ...
    output.push({questionId:q.id,total:rows.reduce((a,b)=>a+b.count,0),counts:rows});
  }
  return output;
}
...
app.get("/health", async (_req,res)=>{
  try { await db("SELECT 1"); res.json({ok:true,app:"QuizFreeOnline",version:"2.0.0"}); }
...
app.get("/api/sessions/:id", async (req,res)=>{
  ...
  res.json({id:s.id,title:s.title,activeQuestion:s.active_question,questions:s.questions.map(publicQuestion)});
});
...
app.post("/api/sessions/:id/questions/import", upload.single("file"), async (req,res)=>{
  ...
  const normalize=r=>{
    ...
    return {text:String(text).trim(),type,options:[...new Set(options)]};
  };
  const imported=rows.map(normalize).filter(q=>q.text&&q.options.length>=2);
  if(!imported.length) return res.status(400).json({error:"No encontré preguntas válidas. Revisa question/type/options."});
  ...
  try {
    ...
    for(let i=0;i<imported.length;i++){
      const q=imported[i];
      await client.query("INSERT INTO questions(session_id,position,text,type,options_json) VALUES($1,$2,$3,$4,$5)",[s.id,i,q.text,q.type,JSON.stringify(q.options)]);
    }
...
async function responseRows(sessionId){
  return (await db(`SELECT q.position+1 AS question_number,q.text AS question,r.participant,r.answer,r.created_at FROM responses r JOIN questions q ON q.id=r.question_id WHERE r.session_id=$1 ORDER BY q.position,r.id`,[sessionId])).rows;
}
...
app.get("/api/sessions/:id/results.csv", async (req,res)=>{
  ...
  const csv=["question_number,question,participant,answer,created_at",...rows.map(r=>[r.question_number,r.question,r.participant,r.answer,r.created_at].map(esc).join(","))].join("\n");
...
app.get("/api/sessions/:id/results.xlsx", async (req,res)=>{
  ...
  const ws=XLSX.utils.json_to_sheet(rows.map(r=>({Pregunta_N:r.question_number,Pregunta:r.question,Participante:r.participant,Respuesta:r.answer,Fecha:r.created_at})));
...
io.on("connection",socket=>{
  ...
  socket.on("answer", async ({sessionId,questionId,participant,answer})=>{
    const s=await getSession(sessionId); if(!s||!questionId||answer==null) return;
    ...
    await db("INSERT INTO responses(session_id,question_id,participant,answer) VALUES($1,$2,$3,$4)",[sessionId,q.id,String(participant||"Anónimo").slice(0,80),normalized.slice(0,1000)]);
...
initDb().then(()=>server.listen(port,()=>console.log(`QuizFreeOnline 2.0 escuchando en el puerto ${port}`))).catch(error=>{console.error("No se pudo iniciar la base de datos:",error);process.exit(1);});
